...
import { deleteTestMCQQuery, getAllMCQBasedOnAcademyQuery, getSpecificMCQQuery, searchMCQBasedOnFiltersQuery, updateTestMCQQuery } from "../../sqlQueries/reusedSQLQueries";
...
export async function GetSpecificMCQ(
  ...
  const specificMCQ = await pgPool.query<UpdateMcqInput>(getSpecificMCQQuery, [id])
...
export async function GetAllMCQBasedOnAcademy(
  ...
  const allMcqsOfAcademy = await pgPool.query(
    getAllMCQBasedOnAcademyQuery, [academyName, offset])
