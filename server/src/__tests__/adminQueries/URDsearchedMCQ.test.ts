...
describe("test the get spcific mcq from the table functionlity", () => {
  ...
  it("should return the required mcq based on the correct id", async () => {
    ...
    expect(response.body.data.GetSpecificMCQ).toEqual(expect.objectContaining({
  ...
  it("nagetive : should fail to get the mcq if the id is incorrect", async () => {
    getSpecificMcqQuery.variables.id = 10000000
    const response = await apiPost(getSpecificMcqQuery)
    expect(response.body.errors).toHaveLength(1)
    expect(response.body.data.GetSpecificMCQ).not.toBeDefined()
  })
})
...
describe("tests the update already present mcq correctly", () => {
  ...
  it("should fail to update the mcq if the id is incorrect", async () => {
    updateMCQQuery.variables.input.id = 10000000
    const error = await apiPost(updateMCQQuery)
    expect(error.body.data.UpdateTestMCQ).not.toBeDefined()
    expect(error.body.data.errors).toHaveLength(1)
  })
})
...
describe("test the deletion of a specific mcq from the database", () => {
  ...
  it("should fail to delete the mcq if the id is incorrect", async () => {
    deleteMCQQuery.variables.id = 10000000
    const response = await apiPost(deleteMCQQuery)
    expect(response.body.data.deleteMCQQuery).not.toBeDefined()
    expect(response.body.errors).toHaveLength(1)
  })
})
...
describe("tests the get mcqs based on the academy name and offset using pagination", () => {
  ...
  it("should return the paginate based mcqs", async () => {
    ...
    expect(response.body.data.GetAllMCQBasedOnAcademy).toHaveLength(10)
    expect(response.body.data.GetAllMCQBasedOnAcademy[0]).toEqual(expect.objectContaining({
      id: expect.any(Number),
      statement: expect.any(String)
    }))
  })

  it("should fail to give the paginate mcqs if the academy name is incorrect", async () => {
    getPaginatedMCQQuery.variables.academyName = "hola"
    const error = await apiPost(getPaginatedMCQQuery)
    expect(error.body.data.GetAllMCQBasedOnAcademy).not.toBeDefined()
    expect(error.body.data.errors).toHaveLength(1)
  })
})
