...
function websiteConfig(configName) {
    if (configName === "auth")
        return { appTitle: "Authentication", siteRoot: "/auth" };
    else if (configName === "user")
        return { appTitle: "User", siteRoot: "/user" };
    else if (configName === "admin")
        return { appTitle: "Admin", siteRoot: "/admin" };
    else if (configName === "tokenManager")
        return { appTitle: "Token Manger", siteRoot: "/token-manager" };
    else
        throw new Error("incorrect config name provided");
}
